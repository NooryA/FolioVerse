...
export default function Home() {
  ...
  const fileSystem: { [key: string]: FileSystemItem[] } = {
    ...
    "Desktop/Portfolio/Projects": [
      ...
      { id: "16", name: "AI Chat Assistant", type: "folder", icon: <Cpu className="w-8 h-8" />, dateModified: "2024-01-13" },
  ...
  return (
    <div className="h-screen bg-gray-100 flex flex-col font-segoe">
      ...
      <div className="flex-1 bg-white border border-gray-300 shadow-lg mx-2 my-2 flex flex-col">
        ...
        <div className="flex-1 flex">
          ...
          {showDetails && selectedItem && (
            <div className="w-80 border-l border-gray-200 bg-gray-50 p-4">
              {(() => {
                const item = getCurrentItems().find((i) => i.id === selectedItem);
                if (!item) return null;
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-center mb-4">
                        <div className="w-16 h-16">{getFileIcon(item)}</div>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <div className="text-sm text-gray-900">
                          {item.type === "folder" ? "Folder" : `${item.extension?.toUpperCase() || "File"} File`}
                        </div>
                      </div>
                      {item.size && (
                        <div>
                      ...
                      <div>
                        ...
                        <div className="text-sm text-gray-900">{formatDate(item.dateModified)}</div>
                      </div>
                      {item.content && (
                        <div>
                          <label className="text-xs font-medium text-gray-500">Preview</label>
                          <div className="text-xs text-gray-700 bg-white p-2 rounded border max-h-32 overflow-auto whitespace-pre-wrap">
        ...
        <div className="bg-gray-50 border-t border-gray-200 px-4 py-1 flex items-center justify-between text-xs text-gray-600">
          <div>{getCurrentItems().length} items</div>
          <div className="flex items-center gap-4">
            <div>{getCurrentItems().filter((i) => i.type === "folder").length} folders</div>
            <div>{getCurrentItems().filter((i) => i.type === "file").length} files</div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-gray-800 text-white px-4 py-2 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div className="flex gap-2">
            <button className="p-2 bg-blue-600 border-2 border-blue-400 rounded">
              <Folder className="w-5 h-5" />
            </button>
